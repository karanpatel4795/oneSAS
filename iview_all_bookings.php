          ...
          <div class="title-holder">
            <div class="title-holder-cell text-left">
              ...
              <ol class="breadcrumb">
                ...
                <li class="active">View All Booking</li>
              </ol>
            </div>
          </div>
        </div>
  ...
  <div class="container">
      ...
      <div class="col-sm-12 col-md-12">
        <div class="product-table">
          <table class="table">
            ...
            <tbody>
           <?php
	$query = "select providers.category,booking.sr, booking.canbyproviderreasone, booking.providername, booking.time, booking.comp_time, booking.canbyuserreasone, booking.canbyuser, booking.canbyprovider,booking.completeorder from providers, booking where providers.email = booking.providername and uname = '$uname'";
	...
	$result = $con->query($query);
		...
		while($row = $result->fetch_assoc())
		{
			...
			$proname = $row["providername"];	
			$time = $row["time"];
			
			if($row["completeorder"] == 1)
			{
				$status = "<a href='prov_feedback.php?provider=$row[providername]'><b>Feedback</b></a>";
				/*$query1 = "select * from ratting where provider = '$proname' and time = $time";
					...
					{
						$reason = "Feedback not Provided";	
					}*/
					$reason = " -- ";
					$comp_time = $row["comp_time"];
			}
			else if($row["canbyuser"] == 1)
			{
				$status = "Cancelled by You";
				$reason = $row["canbyuserreasone"]; 
				$comp_time = $row["comp_time"];
			}
			else if($row["canbyprovider"] == 1)
			{
				$status = "Cancelled by Provider";
				$reason = $row["canbyproviderreasone"]; 
				$comp_time = $row["comp_time"];
			}
			else
			{
      ...
      <div class="col-md">
        <div class="side_bar">
          ...
          <h4>Other Links</h4>
            <div class="categary">
              <ul>
                <li><a href="view_pending_bookings.php"><i class="fa fa-angle-right"></i> View Pending Orders</a></li>
