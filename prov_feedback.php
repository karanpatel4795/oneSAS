<?php
	...
	$pname = $_GET["provider"];
	$query = "select * from providers where email = '$pname'";
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 appointment_form">
              <div class="form_section">
                ...
                <div class="field col-lg-12 col-md-12 col-sm-12 col-xs-12">
                      <textarea class="field_custom" placeholder="Feedback" name="feedback" required rows="10" maxlength="500"></textarea>
                    </div>
                
                  Select Ratting for Provider : 
                  <div class="field col-lg-6 col-md-6 col-sm-12 col-xs-12">
                      <input class="field_custom" placeholder="select appropriate number" type="number" name="rate" required max="5" min="1" />
                    </div>
                  <br><br>
                  <div class="field col-lg-12 col-md-12 col-sm-12 col-xs-12">
                      <input type="checkbox" name="feedbacktoadmin"/> If you want to give Feedback to direct <b>Admin</b> for any misbehaviour of the provider checked it.
                    </div>
                    <hr><br><br>
                 <div class="center">
                    <button class="btn main_bt" value="Submit" name="done">Rate Now</button>
                  </div>
                  </fieldset>	
                </form>
              </div>
...
<?php
	if(isset($_POST["done"]))
	{
		...
		if(isset($_POST["feedbacktoadmin"]))
		{
			$query = "insert into ratting (uname, provider, feedback, rate, feedbacktoadmin) values ('$uname', '$pname', '$feedback' ,$rate, 1)";
		}
		else
		{
			$query = "insert into ratting (uname, provider, feedback, rate) values ('$uname', '$pname', '$feedback' ,$rate)";
		}
		$con->query($query);
		
		$query = "select * from ratting where provider = '$pname'";
		$result = $con->query($query);
		...
		$query = "update providers set rate = $total where email = '$pname'";
		$con->query($query);
